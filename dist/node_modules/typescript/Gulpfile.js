define(["require", "exports", "child_process", "path", "fs", "gulp", "gulp-help", "run-sequence", "gulp-concat", "gulp-clone", "gulp-newer", "gulp-typescript", "gulp-insert", "gulp-sourcemaps", "del", "mkdirp", "minimist", "browserify", "through2", "merge2", "into-stream", "os", "travis-fold", "convert-source-map", "sorcery"], function (require, exports, cp, path, fs, originalGulp, helpMaker, runSequence, concat, clone, newer, tsc, insert, sourcemaps, del, mkdirP, minimist, browserify, through2, merge2, intoStream, os, fold, convertMap, sorcery) {
    "use strict";
    ...
    function exec(cmd, args, complete, error) {
        ...
        ex.on("exit", function (code) { return code === 0 ? complete() : error(undefined, code); });
        ex.on("error", error);
    }
    ...
    gulp.task(builtLocalCompiler, false, [servicesFile], function () {
        var localCompilerProject = tsc.createProject("src/compiler/tsconfig.json", getCompilerSettings({}, true));
    ...
    gulp.task(servicesFile, false, ["lib", "generate-diagnostics"], function () {
        var servicesProject = tsc.createProject("src/services/tsconfig.json", getCompilerSettings({ removeComments: false }, false));
        ...
        var completedJs = js.pipe(prependCopyright())
            .pipe(sourcemaps.write("."));
        var completedDts = dts.pipe(prependCopyright(true))
    ...
    gulp.task(cancellationTokenJs, false, [servicesFile], function () {
        var cancellationTokenProject = tsc.createProject("src/server/cancellationToken/tsconfig.json", getCompilerSettings({}, true));
    ...
    gulp.task(typingsInstallerJs, false, [servicesFile], function () {
        var cancellationTokenProject = tsc.createProject("src/server/typingsInstaller/tsconfig.json", getCompilerSettings({}, true));
        return cancellationTokenProject.src()
    ...
    gulp.task(serverFile, false, [servicesFile, typingsInstallerJs, cancellationTokenJs], function () {
        var serverProject = tsc.createProject("src/server/tsconfig.json", getCompilerSettings({}, true));
    ...
    gulp.task(tsserverLibraryFile, false, [servicesFile], function (done) {
        var serverLibraryProject = tsc.createProject("src/server/tsconfig.library.json", getCompilerSettings({}, true));
    ...
    gulp.task(run, false, [servicesFile], function () {
        var testProject = tsc.createProject("src/harness/tsconfig.json", getCompilerSettings({}, true));
    ...
    gulp.task("runtests", "Runs the tests using the built run.js file. Optional arguments are: --t[ests]=regex --r[eporter]=[list|spec|json|<more>] --d[ebug]=true --color[s]=false --lint=true.", ["build-rules", "tests"], function (done) {
        runConsoleTests("mocha-fivemat-progress-reporter", false, done);
    });
    var nodeServerOutFile = "tests/webTestServer.js";
    var nodeServerInFile = "tests/webTestServer.ts";
    gulp.task(nodeServerOutFile, false, [servicesFile], function () {
        var settings = getCompilerSettings({ module: "commonjs" }, true);
    ...
    gulp.task("browserify", "Runs browserify on run.js to produce a file suitable for running tests in the browser", [servicesFile], function (done) {
        var testProject = tsc.createProject("src/harness/tsconfig.json", getCompilerSettings({ outFile: "../../built/local/bundle.js" }, true));
        return testProject.src()
            ...
            browserify(intoStream(file.contents), { debug: true })
                ...
                var maps = JSON.parse(convertMap.fromSource(res.toString(), true).toJSON());
    ...
    gulp.task("build-rules", "Compiles tslint rules to js", function () {
        var settings = getCompilerSettings({ module: "commonjs" }, false);
